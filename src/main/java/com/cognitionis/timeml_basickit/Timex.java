...
public class Timex extends Element implements Cloneable {
    ...
    @Override public Timex clone() {
        try {return (Timex) super.clone();} catch (CloneNotSupportedException e) {throw new AssertionError();}
    }
    ...
    /**
     * Given a String TimeML date/time value it returns a propver ISO/Java date/time.
     * It replaces WI, SU, MO, AF etc.
     *
    ...
    public void get_interval_values() {
        ...
        if(lv==null || uv==null){
            ...
            if (value.matches("(?i)[0-9]{4}-W[0-9]{2}(-WE)?")){
                DateTimeFormatter fmt = ISODateTimeFormat.dateOptionalTimeParser();
                Date dateaux = new DateTime(fmt.parseDateTime(value)).toDate();
                GregorianCalendar cal=new GregorianCalendar();
                cal.setTime(dateaux);
                lv = new DateTime(cal.getTime()).toString(granul_days) + "T00:00:00";
                cal.add(GregorianCalendar.DAY_OF_MONTH, 6);
                uv = new DateTime(cal.getTime()).toString(granul_days) + "T23:59:59";
